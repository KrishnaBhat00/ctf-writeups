...
We can deal with this by using the same trick that JSFuck does, finding ways for JS to give us a `C` and then keeping it for ourselves. If we can render `document.embeds`, we can use the `[object HTMLCollection]` string it gives us, and slice it down to the `C`. The DOM property is too expensive to declare so we still need to build a string and `eval` it.
